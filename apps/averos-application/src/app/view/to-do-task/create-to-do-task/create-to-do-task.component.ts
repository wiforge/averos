...
export class CreateToDoTaskComponent implements CreateViewEditUseCase<ToDoTask>, OnInit, OnDestroy {
  ...
  componentNewValue: IndexableType<ToDoTask> = new ToDoTask()
        ...
        case UseCaseAction.DELETE:
          this.deleteRelationSubscription = this.entityService
            .deleteRelationCollection(
              ...
              this.useCaseConfig.entity as ToDoArea | Partial<ToDoArea>,
            ...
            .subscribe({
              next: (updates: any) => {
                this.alertService
                  .success($localize`:@@uc.update.entity:Entity ${(this.useCaseConfig.entity as Indexable)[TypeScriptTypeMetaDatatHandler.instance.getBusinessIdName(this.useCaseConfig.entityType)]}:entity:
              ...
              error: (err: Error) => {
                console.log(err)
                this.alertService
                  .error($localize`:@@uc.update.entity.error:Entity ${(this.useCaseConfig.entity as Indexable)[TypeScriptTypeMetaDatatHandler.instance.getBusinessIdName(this.useCaseConfig.entityType)]}:entity:
                      cannot be updated`)
                this.deleteRelationSubscription?.unsubscribe()
              },
            })
  ...
  onSubmit(submittedValue: IndexableType<ToDoTask>) {
    ...
    if (this.useCaseConfig.useCase === UseCase.CREATE) {
      this.createEntitySubscription = this.entityService
        ...
        .subscribe({
          next: (submittedEntity: ToDoTask | null) => {
            this.alertService
